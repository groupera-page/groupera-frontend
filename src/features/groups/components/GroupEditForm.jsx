...
let GroupEditForm = ({ handleSubmit, reset, group }) => {
  ...
  return (
    <form
      ...
      className="my-2 lg:grid grid-cols-2 gap-12 gap-y-5 lg:mx-0 items-start"
    >
      ...
      <div className={"col-span-2"}>
        {group.meetings.map((meeting, idx) => {
          return (
            <div
              key={idx}
              className="bg-BG_PRIMARY rounded-md border border-BORDER_PRIMARY p-4 my-4 "
            >
              <ul className="flex justify-between items-end paragraph-lg">
                <li>
                  <ul className="flex flex-wrap gap-2 lg:gap-12 items-center">
                ...
                <li>
                  <ul className="flex gap-4 items-end">
                    <li className="cursor-pointer text-TEXT_LIGHTGRAY hover:text-PURPLE_PRIMARY">
                      <IoSettingsOutline
                        size={32}
                        onClick={() => navigate(`event/${meeting.id}`)}
                      />
                    </li>
                    {/*<li className="cursor-pointer text-TEXT_LIGHTGRAY hover:text-PURPLE_PRIMARY">*/}
      ...
      <div className="flex gap-4 col-span-2 justify-end">
        <SecondaryButton
          isLarge={true}
          handleButtonClick={() => reset() && navigate(`/groups/${groupId}`)}
        >
          Abbrechen
        </SecondaryButton>
